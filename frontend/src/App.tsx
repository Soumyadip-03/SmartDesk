...
export default function App() {
  ...
  const handleUserNameUpdate = (newName: string) => {
    ...
    localStorage.setItem(`${userId}_fullName`, newName);
  };
  ...
  return (
    <ErrorBoundary>
      ...
      <div className="relative z-10 p-6">
        ...
        <header className="flex items-center justify-between mb-8">
          <div className="flex items-center gap-4">
            ...
            <h1 className="text-xl font-semibold text-white">SmartDesk</h1>
          </div>
          ...
          <div className="flex items-center gap-4">
            ...
            >
              <Heart className={`w-5 h-5 ${wishlistRooms.length > 0 ? "text-red-400" : "text-gray-400"}`} />
        ...
        <div className="flex justify-end mb-8">
          <div className="bg-white/10 backdrop-blur-sm rounded-lg px-4 py-2 border border-white/10">
            <span className="text-white/80 text-sm">{formatDate(currentDate)}</span>
          </div>
        </div>

        {/* Buildings Section */}
        <div className="mb-8">
          <div className="bg-gradient-to-r from-white/20 to-white/10 backdrop-blur-sm px-4 py-2 border border-white/10 inline-block" style={{borderRadius: '3rem'}}>
            <span className="text-white text-sm font-medium">Buildings</span>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {buildingsLoading ? (
            ...
            <div className="col-span-full flex justify-center items-center h-32">
              <div className="text-white/60">No buildings found</div>
            </div>
          ) : (
      ...
      {isMenuOpen && (
        ...
        >
          <div 
            className="absolute top-20 right-6 w-72 bg-gradient-to-br from-white/20 to-white/10 backdrop-blur-xl rounded-3xl p-6 shadow-2xl border border-white/20"
            onClick={(e) => e.stopPropagation()}
          >
            {/* Menu Header */}
            <div className="text-center mb-6">
              <div className="w-16 h-16 bg-gradient-to-br from-blue-500/20 to-purple-500/20 rounded-full flex items-center justify-center mx-auto mb-3 border border-white/20">
                <User className="w-8 h-8 text-white/80" />
              </div>
              <h3 className="text-white font-medium">Welcome Back</h3>
              <p className="text-white/60 text-sm">{getFirstName(userName) || "Guest User"}</p>
            </div>
